...
export default function SmeAssignPage() {
  ...
  const fetchData = useCallback(async (opts?: { smeQuery?: string }) => {
    try {
      ...
      if (!smeResp.ok) throw new Error("Failed to load SMEs");
          ...
          <CardContent>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1">
                <div className="mb-2 text-sm text-gray-400">Select SME</div>
                <div className="relative mb-3">
                  ...
                  <Input
                    ...
                    placeholder="Search SME by name/email"
                    className="pl-9 pr-20 bg-gray-900 border-gray-800 text-white placeholder:text-gray-500"
                  />
                  <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    {smeSearch && (
                      <button
                        type="button"
                        onClick={() => setSmeSearch("")}
                ...
                <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
                  {filteredSmes.map((s) => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg border ${selectedSmeId === s.uid ? 'border-blue-500 bg-blue-500/10' : 'border-gray-800 bg-gray-900/50 hover:bg-gray-900'}`}
                    >
                  ...
                  {filteredSmes.length === 0 && (
                    <div className="text-xs text-gray-500 p-2">No SMEs found</div>
                  )}
                </div>
              </div>
              ...
              <div className="lg:col-span-2">
                <div className="mb-2 flex items-center justify-between text-sm text-gray-400">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      checked={filteredProducts.length > 0 && filteredProducts.every(p => !!selectedProductIds[p.productId])}
                      onChange={(e) => {
                        const checked = e.target.checked;
                        setSelectedProductIds(prev => {
                          const next = { ...prev } as Record<string, boolean>;
                          filteredProducts.forEach(p => { next[p.productId] = checked; });
                          return next;
                        });
                        setSelectedQuantities(prev => {
                          const next = { ...prev } as Record<string, string>;
                          filteredProducts.forEach(p => { if (checked) next[p.productId] = next[p.productId] ?? "1"; });
                          return next;
                        });
                      }}
                    />
                ...
                <div className="max-h-80 overflow-y-auto divide-y divide-gray-800 border border-gray-800 rounded-lg">
                  ...
                  {filteredProducts.length === 0 && (
                    <div className="text-xs text-gray-500 p-3">No products found</div>
                  )}
                </div>
